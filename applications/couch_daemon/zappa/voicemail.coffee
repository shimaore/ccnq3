@include = ->
  ...
  @put '/_ccnq3/voicemail/:number@:number_domain', ->
    ...
    if not @req.user?
      return @send error:"Not authorized (probably a bug)"
    ...
    provisioning.retrieve id, (e,r,local_number) =>
      ...
      if not user_database?
        ...
        provisioning.update local_number, (e) =>
          if e? then return @send error:e, when:"local_number #{id}"
          do step2
      else
        do step2
      ...
      step2 = =>
        if not user_database.match /^u[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/
          return @send error:"Invalid db name #{user_database}"
        ...
        users_db.view 'replicate', 'userdb', qs: {key:JSON.stringify user_database}, (e,r,b) =>
          if e? then return @send error:e, when:"view users for #{user_database}"
          ...
          target_db.create =>
            ...
            target_db.request.put '_revs_limit',body:"10", (e,r,b) =>
              if e? then return @send error:e, when:"set revs_limit for #{user_database}"

              # Make sure the users can access it.
              target_db.request.get '_security', json:true, (e,r,b) =>
                if e? then return @send error:e, when:"retrieve security object for #{user_database}"
                ...
                target_db.request.put '_security', json:b, (e,r,b) =>
                  if e? then return @send error:e, when:"update security object for #{user_database}"
                  ...
                  target_db.get 'voicemail_settings', (e,r,vm_settings) =>
                    ...
                    target_db.update vm_settings, (e) =>
                      if e? then return @send error:e, when:"update voicemail_settings for #{user_database}"
                      @send
                        ok:true
