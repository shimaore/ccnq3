...
=head1 SYNOPSIS
   ...
   my $dbh = new AE::DBI "DBI:SQLite:dbname=test.db", "", "";
...
argument. All callbacks used by this module get their AE::DBI handle
...
sub serve_fh($$) {
   ...
   if ($VERSION != $version) {
      syswrite $fh,
         pack "L/a*",
            Storable::freeze
               [undef, "AE::DBI version mismatch ($VERSION vs. $version)"];
      return;
   }
...
=item $dbh = new AE::DBI $database, $user, $pass, [key => value]...
...
When on_connect is supplied, connect error are not fatal and AE::DBI
...
AE::DBI proxy running. This will provide the cleanest possible proxy
...
sub new {
   ...
   my ($client, $server) = AnyEvent::Util::portable_socketpair
      or croak "unable to create AE::DBI communications pipe: $!";
   ...
   if ($pid) {
      ...
      if ($self->{exec_server}) {
         ...
         exec {$^X}
              "$0 dbi slave",
              -e => "require shift; AE::DBI::serve_fd ($serv_fno, $VERSION)",
              $INC{"AE/DBI.pm"};
         POSIX::_exit 124;
      } else {
...
The callback will be called with a weakened AE::DBI object as the
...
between the AE::DBI server process and your program does not support the
passage of closures.
